    ...
    \begin{homeworkProblem}
        ...
        \begin{enumerate}
            ...
            \begin{itemize}
                ...
                \end{tabular}\\
                $\to \sum\limits_{i=0}^{k} 2^i \cdot \frac{n}{2^i}\log \frac{n}{2^i}$\\
                $\to \sum\limits_{i=0}^{\log_2 n} n\log \frac{n}{2^i}$\\
                $\to \sum\limits_{i=0}^{\log_2 n} n\log n - n\log 2^i$\\
                $\to n\log n\log n - \sum\limits_{i=0}^{\log_2 n} ni$\\
                $\to n\log n\log n - n\frac{\log_2 n(\log_2 n + 1)}{2}$\\
                $\to n\log n\log n - \frac{n\log_2 n\log_2 n}{2} - \frac{n\log_2 n}{2}$\\
                $\to \frac{n\log_2 n\log_2 n}{2} - \frac{n\log_2 n}{2}$\\
            ...
            They are smaller than $T_1$, which is $\Theta(n^2)$\\
        \end{enumerate}
    \end{homeworkProblem}
...
MT(low):
    ...
    return memo[low]
            ...
            Since the iterative solution iterates through the array indexing each element once,
            it's $O(n)$.\\
        \end{enumerate}
    \end{homeworkProblem}
\end{document}
