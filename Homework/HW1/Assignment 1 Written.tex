...
\begin{document}
    ...
    \begin{homeworkProblem}
        \textbf{(6 points)}\newline
        \textit{For this problem, I would use a modified binary search where the I would have the
        find the appropriate initial search space as the bounds are unknown. I know that the
...
while (course needs TA) {
    ...
    if (ta is unqualified for c or ta doesn't want to teach c) {
        c rejects ta
        ...
        ta rejects c
            ...
            \begin{proof}
                The sorted list of applicant TAs for each course is strictly decreasing each
                ...
                Since there are more TAs than total course spots, there must be a TA t$_0$ that is
                unassigned.\newline
                Since t$_0$ is unassigned, then c$_0$ must have rejected t$_0$, however, since t$
                _0$ is unassigned, then t$_0$ must be unqualified or didn't want to
                teach c$_0$.\newline
                $\therefore$ we have a contradiction, and all available TA spots are filled
                unless unqualified or the TA doesn't want to teach the class.\newline
