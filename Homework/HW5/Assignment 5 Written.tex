...
def min_palindrome(s):
    ...
    for j in [1, len(s)]:
        ...
        sections[j] = min_sections
            ...
            We know OPT(k + 1) = min(OPT(k) + 1, min(OPT(j - 1) + 1 for j in [1, k] if isPali(j, k + 1)))\\
    ...
    \begin{homeworkProblem}
        ...
        \begin{enumerate}
            ...
            enjoyment.\\
            $e_i$ = enjoyment if person can be included\\
            $e_e$ = enjoyment if person can't be included\\
            \begin{lstlisting}
...
def max_enjoyment(person, canInclude) -> int:
    ...
    Map(person, {ci, ce})
    return canInclude ? ci : ce
            \end{lstlisting}
            Let i be the level where i = 0 is the root\\
            The optimal solution is OPT(i) = max(OPT(i + 1), OPT(i + 2))\\
