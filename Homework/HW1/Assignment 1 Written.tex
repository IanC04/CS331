...
\begin{document}
    ...
    \begin{homeworkProblem}
        ...
        \begin{proof}
            \begin{tabular}{|lr|}
                \hline
                a$_{k+1}$ = a$_{k}$ + 2a$_{k-1}$                   & Inductive Hypothesis \\
                = (3 $\cdot$ 2$^{k-1}$ + 2(-1)$^{k}$) + 2a$_{k-1}$ & Factor out 2         \\
                = 2(3 $\cdot$ 2$^{k-2}$ + 2(-1)$^{
...
while (low <= high) {
    ...
    if (E[mid] == x) return mid;
        ...
        \begin{proof}
            ...
            The second while loop is a binary search, which is O($\log n$).\newline
            The first part of the algorithm terminates since the high value is doubled each
            iteration, and won't exceed the length of the array.\newline
            The second part of the algorithm terminates since the range of the search is halved
            each iteration.\newline
