...
\begin{document}
    ...
    \begin{homeworkProblem}
        ...
        to each artist.\\
        It means the cost of inviting the artists that perform on it is fulfilled and from the
        stages to the target, the entire cost is consumed, meaning we will at least make one
        dollar.\\
        Take the artists associated with the stages which have flow $<$ capacity to the target.\\
        We then take all stages that have their artists' requirement fulfilled by our chosen subset
        of stages.\\
    \end{homeworkProblem}
    ...
    \begin{homeworkProblem}
        ...
        Layer 4- The target.\\
        This works since the max-flow will push the maximum amount of flow/produce through the
        network, which is constrained based on a store's budget and list of needed items.\\
        The resulting max-flow stores the maximum number of produce able to be sold along with
        the amount to each store.\\
        The algorithm is Ford-Fulkerson, which is $O(mC)$, where m = number of edges and
        C = total quantity of produce supplied by the farm.\\
